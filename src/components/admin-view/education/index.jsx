...
export default function AdminEducationView({
  ...
  const handleDeleteItem = async (id) => {
    const response = await handleDelete("education", id);
    if (response.success) {
      const updateData = data.filter((item) => item._id !== id);
      setAllData((prevData) => ({ ...prevData, education: updateData }));
        ...
        <div className="mb-10 space-y-6">
          {data && data.length ? (
            data.map((item, index) => (
              <div
                key={index}
                className="bg-[#d7d7d7] flex flex-col gap-2 p-6 rounded-lg shadow-md border border-green-600 hover:border-green-800 transition duration-300"
              >
            ...
            <p className="text-center text-gray-600">No Job Education data</p>
          )}
        </div>
